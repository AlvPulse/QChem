...
def main():
    ...
    batch_size = 64
    # Note: For multi-task, simple WeightedRandomSampler is tricky as labels differ per task.
    # We'll rely on class weighting in the loss function instead.
    ...
    # 3. Model Setup
    model = MultiTaskGNN(num_tasks=len(tox21_tasks)).to(device)
    ...
    # Specific task result (SR-MMP is index 10 in the list provided in the notebook)
