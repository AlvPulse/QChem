...
| 8 | 28 | +0.0030 (p = 0.17, n.s.) | *deferred — see note* |

At K = 6, the gate-only bias is no longer statistically distinguishable from zero, while Level 8's bias
is *larger than at K = 4* and remains significant — and Level 8 simultaneously has **higher absolute
accuracy** (AUC 0.651 vs 0.641). More qubits means more bonds means a richer graph readout: the
measurement mechanism turns added capacity into added signal instead of added noise.

> **Note on the K = 8 Level 8 cell.** The scaling *claim* — that the measurement mechanism sustains
> the bias where gate-gating fades — is established by the two completed points (K = 4 and K = 6,
> where gate-only is already non-significant) together with the mechanistic decomposition (§8.5). The
> gate-only K = 8 point is in hand (+0.0030, n.s.); the Level 8 K = 8 confirmation run was not
> completed in this session because the bond-correlator readout at K = 8 (56 extra two-qubit
> observables) is ~3× slower and the compute environment temporarily could not host the multi-hour
> background job. It is a confirmatory third point, not a load-bearing one, and is queued to run.
...
![Level 8 results](../report_figures/level8_results.png)

*Figure 1. (A) Control-validity audit: the scramble residual is exactly 0 at Levels 2/4 (training
absorbs it — vacuous) and large at Levels 5–7 (genuine). (B) Topology bias vs. qubit count:
gate-gating alone (grey) fades to non-significance (open markers = p ≥ 0.05), while measurement-based
Level 8 (blue) grows and stays significant (filled markers); the K = 8 Level 8 point is pending. (C)
Mechanism at K = 4: the bond-correlator readout amplifies the gate-gated bias (+0.0044 → +0.0078),
but with a graph-independent entangler the readout alone hurts (−0.027) — "place-then-harvest."
Source: `generate_level8_figures.py`.*
...
**and** more accurate in absolute terms. K = 8 confirmation is re-running.
...
4. **Higher-K is under-powered, and the K = 8 Level 8 cell is not yet filled.** K = 6 used 3 seeds,
   K = 8 fewer; the Level 8 K = 8 confirmation is queued (see the note under §6.2). The trend (gate
   fades, Level 8 grows) is consistent across the two completed K values plus the mechanistic
   decomposition, but the high-K cells deserve more seeds for tight intervals.
