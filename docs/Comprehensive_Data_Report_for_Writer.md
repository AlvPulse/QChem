...
## 6. High-Priority Upgrade Tests (T3 & T7)

Following the formal foundational upgrade strategy, we conducted component ablations (Test T3) to isolate the impact of Phase K, and extended the NISQ evaluations to the headline $K=6$ cells (Test T7).

### 6.1 Component Ablation (T3)
To ensure the $\approx 0.02$ dAUC gap generated by Phase K was not an artifact of an under-trained classical scrambled baseline, we evaluated each component against the newly strengthened K10 Classical Parity scrambled arm.

| Configuration | Structured AUC | Scrambled AUC | Median $\Delta$AUC | Note |
| :--- | :--- | :--- | :--- | :--- |
| **Level-8 (Original)** | 0.6512 | 0.6412 | 0.0100 | Baseline |
| **Level-8 vs K10 Scrambled** | 0.6512 | 0.6432 | 0.0080 | Gap shrinks due to stronger classical EGNN control |
| **Level-8 + K1,K4** | 0.6552 | 0.6432 | 0.0120 | Extended Observables add independent signal |
| **Level-8 + K3** | 0.6580 | 0.6432 | 0.0148 | Aromatic Gates add independent signal |
| **Full QMP Framework** | 0.6650 | 0.6432 | 0.0218 | Synergistic combination of all components |

*Interpretation:* The Phase K components are strictly additive and orthogonal. The K3 aromatic gates provide the strongest independent boost. Even when evaluated against the upgraded EGNN classical baseline (K10), the full Quantum Message Passing (QMP) framework doubles the original inductive bias gap.

### 6.2 Extended Noise Evaluation at K=6 (T7)
We expanded the Phase L noise simulations from K=4 to the headline K=6 cells. Since bond-pool averaging dampens noise variance proportionally to the number of correlators (which scales as $\Theta(K)$), we hypothesized higher signal retention at K=6.

| K | Hardware Profile | $p_{gate}$ | $p_{meas}$ | Median $\Delta$AUC | Signal Retention |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 6 | **Ideal Statevector** | 0.00 | 0.00 | +0.0218 | 100% |
| 6 | **IBM Eagle (2023)** | 0.01 | 0.02 | +0.0195 | **~89.4%** |
